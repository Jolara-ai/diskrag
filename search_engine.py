...
2. 死鎖風險評估：
   ✅ 單一鎖設計，無死鎖風險
   ✅ 鎖內操作簡單，不會調用其他可能持有鎖的方法
   ✅ 快速釋放鎖，持有時間極短

3. 性能優化建議：

   A. 對於高併發場景：
      - 使用 use_thread_safe_stats=True（默認）
      - 考慮使用專門的監控系統（如 Prometheus）
      - 定期檢查鎖競爭情況

   B. 對於單線程場景：
      - 使用 use_thread_safe_stats=False
      - 可以獲得最佳性能

   C. 性能監控：
      - 使用 performance_test_search_engine() 進行基準測試
      - 監控平均搜索時間和併發加速比
      - 關注統計數據的準確性
...
4. 使用示例：
   ...
   # 線程安全模式（推薦用於 API 服務）
   engine = SearchEngineCorrect("my_collection", use_thread_safe_stats=True)
   
   # 高性能模式（單線程環境）
   engine = SearchEngineCorrect("my_collection", use_thread_safe_stats=False)
   
   # 性能測試
   results = performance_test_search_engine("my_collection", num_queries=100)

5. 預期性能影響：
   - 單次搜索開銷：< 5%（通常 < 1%）
   - 併發加速比：2-4x（取決於 CPU 核心數）
   - 內存開銷：增加一個鎖對象（~64 bytes）

6. 最佳實踐：
   - 在開發環境中進行性能測試
   - 根據實際使用場景選擇線程安全模式
   - 定期監控統計數據的準確性
   - 考慮使用專門的 APM 工具進行性能監控
""" 